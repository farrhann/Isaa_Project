...
long long compute(long long a, long long m, long long n)
{
	long long r;
	long long y = 1;
...
long long gcd(long long int a, long long int h)
{
	long long temp;
	while (1)
	{
...
void decryption()
{
	...
	lltoarrayc(C, ckey, civ);
	cout<<"Decrypted key is:\n";
	for (j = 0; j < 32; j++)
	{
		ckey[j] = compute(ckey[j], d, n);
		cout<<ckey[j]<<" ";
	}
	// cout<<"\n"<<"Decrypted iv is:\n";
	for(j = 0; j < 24; j++){
		civ[j] = compute(civ[j], d, n);
		// cout<<civ[j]<<" ";
	}
	ofstream outfile;
	...
	if (outfile.is_open())
	{
		for (int i = 0; i < 56; i++)
		{
			if(i<32) outfile << ckey[i];
			else outfile<< civ[i-32];
			outfile << " ";
		}
		outfile.close();
...
int main()
{
	...
	if (keyfile.is_open())
	{
		...
		cout << "Reading the key from Key_Cipher.txt" << endl;
		keyfile.close();
	}
	...
	string msgstr;
	ifstream msgfile;
	msgfile.open("ciphertext.txt", ios::in | ios::binary);
	if(msgfile.is_open()){
		getline(msgfile, msgstr);
		cout<<"Reading the encrypted message from ciphertext.txt"<<endl;
		msgfile.close();
	}
	else cout<<"Unable to open ciphertext file";
	uint8_t ciphertext[1000];
	int text_len = strtoarrayc(msgstr, ciphertext);
	// Free memory
	// delete[] msg;
	...
	// for(int i=0;i<32;i++) printf("%d", keyi[i]);
	// cout<<"\n";
	// for(int i=0;i<24;i++) printf("%d", ivi[i]);
	// cout<<"\n";
    xchacha_keysetup(&ctx, keyi, ivi);
	xchacha_decrypt_bytes(&ctx, ciphertext, buffer2, text_len);
	cout<<"Decrypted text is:";
	// cout<<buffer2;
	// cout<<text_len;
	for(int i=0;i<text_len;i++) printf("%c", buffer2[i]);
	return 0;
}